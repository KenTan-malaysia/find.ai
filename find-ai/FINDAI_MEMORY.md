...
> **Last updated:** 2026-04-26 (v3.4.21 — **LHDN verify switched to deep-link pattern** (same as TNB Account #). Tap "🔗 Verify on LHDN STAMPS" → opens stamps.hasil.gov.my in new tab AND immediately marks LHDN verified with mock data. No API negotiation needed — uses LHDN's public Pengesahan Ketulenan portal. Removed unused `verifying` state + `verifyWithLHDN` function. Cakap 2.0 · DNA: TRUST BEFORE SIGNING).
